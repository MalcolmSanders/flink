...
/**
 * {@link CheckpointCoordinator} components in {@link HighAvailabilityMode#KUBERNETES}.
 */
public class KubernetesCheckpointRecoveryFactory implements CheckpointRecoveryFactory {
	...
	public KubernetesCheckpointRecoveryFactory(
		...
		Executor executor) {
		this.apiContext = checkNotNull(apiContext, "Kubernetes client");
	...
	public CompletedCheckpointStore createCheckpointStore(JobID jobId, int maxNumberOfCheckpointsToRetain, ClassLoader userClassLoader) throws Exception {
		return KubernetesUtils.createCompletedCheckpointStore(
			apiContext, config, jobId, maxNumberOfCheckpointsToRetain, executor);
	}

	@Override
	public CheckpointIDCounter createCheckpointIDCounter(JobID jobId) throws Exception {
		return KubernetesUtils.createCheckpointIDCounter(apiContext, config, jobId);
	}
}
